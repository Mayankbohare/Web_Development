...
def insert_order(connection, order):
    ...
    order_query = ("INSERT INTO orders "
             "(coustmer_name, total, datetime)"
             "VALUES (%s, %s, %s)")
    order_data = (order['coustmer_name'], order['grand_total'], datetime.now())
...
def get_all_orders(connection):
    ...
    response = []
    for (order_id, coustmer_name, total, dt) in cursor:
        response.append({
            'order_id': order_id,
            'coustmer_name': coustmer_name,
            'total': total,
            'datetime': dt,
        })
